...
//        force_joint() : ModelPlugin(){}
        ...
        {
//            std::cout << "Loading the Plugin module in ROS" << std::endl;
//            ROS_INFO("Plugin load function called");
            this->model = _parent;
            ...
            for(physics::Joint_V::iterator jit=jointVector.begin(); jit!=jointVector.end(); ++jit)
            {
                ...
                //if not, ignore joint
                std::string modelStr = (*jit)->GetName();
                if((*jit)->GetType() != 576 || modelStr.find("ur10") == std::string::npos)
                    continue;
