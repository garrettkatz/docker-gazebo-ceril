...
class GazeboEnvironment():
    def __init__(self, start_init_physics_parameters, reset_world_or_sim):
        ...
        rospy.loginfo("poppy_application"+"Waiting for service " + str(service_name))
        rospy.wait_for_service(service_name)
        rospy.loginfo("poppy_application"+"Service Found " + str(service_name))
    ...
    def update_gravity_call(self):
        ...
        rospy.loginfo("poppy_application"+str(set_physics_request.gravity))

        result = self.set_physics(set_physics_request)
        rospy.loginfo("poppy_application"+"Gravity Update Result==" + str(result.success) + ",message==" + str(result.status_message))
